...
try:
    from services.admin_bootstrap import clear_temporary_admin_state, is_temporary_admin_claims
    from services.admin_store import AdminAccountLockError, LastAdminRemovalError, PremiumWindowValidationError, UserNotFoundError, get_auth_user, get_auth_user_summary, has_admin_account, list_auth_users_page, update_auth_user_admin_fields
    from services.auth_credit_store import add_user_credits, list_user_credit_history_page
    ...
    from services.request_store import RequestStoreError, list_recent_request_records, list_user_request_records_page
except ImportError:
    from ..services.admin_bootstrap import clear_temporary_admin_state, is_temporary_admin_claims
    from ..services.admin_store import AdminAccountLockError, LastAdminRemovalError, PremiumWindowValidationError, UserNotFoundError, get_auth_user, get_auth_user_summary, has_admin_account, list_auth_users_page, update_auth_user_admin_fields
    from ..services.auth_credit_store import add_user_credits, list_user_credit_history_page
...
def register_admin_routes(app):
    ...
    def update_admin_user_route(user_id):
        ...
        next_premium_start_at = payload.get('premiumStartAt') if 'premiumStartAt' in payload else None
        next_premium_end_at = payload.get('premiumEndAt') if 'premiumEndAt' in payload else None
        next_is_locked = payload.get('isLocked') if 'isLocked' in payload else None
        if next_role is None and next_credits is None and next_is_premium is None and next_premium_start_at is None and next_premium_end_at is None and next_is_locked is None:
        ...
        try:
            updated_user = update_auth_user_admin_fields(
                ...
                premium_start_at=next_premium_start_at,
                premium_end_at=next_premium_end_at,
        ...
        except PremiumWindowValidationError as error:
