...
try:
    ...
    from repositories.iap_bank_hook_history_repository import (
        count_iap_bank_hook_history_rows,
        ensure_iap_bank_hook_history_table,
        get_iap_bank_hook_history_row,
        insert_iap_bank_hook_history_row,
        list_iap_bank_hook_history_rows,
    )
except ImportError:
    from ..utils.pagination import build_pagination, normalize_pagination
...
def get_iap_bank_hook_history(history_id):
    ...
    try:
        row = get_iap_bank_hook_history_row(history_id)
        if not row:
...
def record_iap_bank_hook_history(api_key_record, payload):
    ...
    try:
        history_id = insert_iap_bank_hook_history_row(
            {
                'api_key_id': int(api_key_record.get('id') or 0),
                'api_key_name': _normalize_text(api_key_record.get('name'), max_length=120),
                'gateway': _normalize_text(safe_payload.get('gateway'), max_length=120),
                'transaction_at': transaction_at,
                'transaction_date_text': transaction_date,
                'account_number': _normalize_text(safe_payload.get('accountNumber'), max_length=80),
                'sub_account': _normalize_text(safe_payload.get('subAccount'), max_length=80),
                'code': _normalize_text(safe_payload.get('code'), max_length=120),
                'content': _normalize_text(safe_payload.get('content'), max_length=2000) or None,
                'transfer_type': _normalize_text(safe_payload.get('transferType'), max_length=32),
                'description': _normalize_text(safe_payload.get('description'), max_length=2000) or None,
                'transfer_amount': _normalize_integer(safe_payload.get('transferAmount')),
                'accumulated': _normalize_integer(safe_payload.get('accumulated')),
                'reference_code': _normalize_text(safe_payload.get('referenceCode'), max_length=160),
                'payload_json': payload_json,
                'created_at': safe_created_at,
            }
        )
        return get_iap_bank_hook_history(history_id)
    except driver.MySQLError as error:
...
def list_iap_bank_hook_history_page(page=1, page_size=20, search_term='', start_date='', end_date=''):
    ...
    try:
        ...
        return {
            'history': [_row_to_bank_hook_history(row) for row in rows],
