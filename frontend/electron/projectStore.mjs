...
function buildProjectRecord(projectId, payload, existingRecord = null) {
  ...
  return {
    ...
    series_id: payload.videoSeriesId ?? existingRecord?.series_id ?? '',
    series_name: payload.videoSeriesName ?? existingRecord?.series_name ?? '',
    episode_number: payload.episodeNumber ?? existingRecord?.episode_number ?? 0,
...
function toProjectSummary(projectRecord) {
  return {
    ...
    series_id: projectRecord.series_id || '',
    series_name: projectRecord.series_name || '',
    episode_number: projectRecord.episode_number || 0,
